...
class SurveyPage : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val etWeight = findViewById<EditText>(R.id.weight)
        ...
        btnSubmit.setOnClickListener {
            ...
            val weight = etWeight.text.toString().trim().toDoubleOrNull() ?: 0.0
            val password = etPassword.text.toString().trim()

            sendUserData(email, password, firstName, lastName, dob, weight)
        }
    }
    ...
    private fun sendUserData(
        ...
        OkHttpClient().newCall(request).enqueue(object : Callback {
            override fun onFailure(call: Call, e: IOException) {
                runOnUiThread {
                    Toast.makeText(this@SurveyPage, "Network error: ${e.message}", Toast.LENGTH_LONG).show()
                }
            }
